...
export class FormValidationService {
  ...
  getErrorMessage(fieldName: string, targetForm: any): string {
    ...
    return formField?.hasError('required')
      ...
      ? 'Passwords mismatch'
      :formField?.hasError('nonIdentical')
      ? 'Choisir parmis les établissements'
      :formField?.hasError('nonIdentical_filiere')
      ? 'Choisir parmis les Filieres'
      :formField?.hasError('mismatch_type')
      ? 'only number allowed'
      : 'Unknown error';
  }
